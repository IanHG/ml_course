...
\begin{tikzpicture}
   ...
   % Draw inputs
   \node[input] (I-0) at (-\layersep, 1) {$h^{(i)}_0 = 1$};
   \node[annot,text=blue,left of=I-0, node distance=1.2cm] (o) {Fixed};
   
   \node[input] (I-1) at (-\layersep, 0) {$h^{(i)}_1$};
   \node[input] (I-2) at (-\layersep,-1) {$h^{(i)}_2$};
   \node[input] (I-M) at (-\layersep,-3) {$h^{(i)}_m$};
   ...
   \node[activation] (ACT) at (2 * \layersep, -1.5) {$f \left( . \right)$};
   \node[annot,below of=ACT, node distance=0.82cm] (af) {Activation function};

   % Draw output
   \node[input] (O-1) at (3 * \layersep, -1.5) {$h^{(o)}_k$};
   ...
   \path (SUM) edge[arrow,line width=0.3mm] node [above,midway,font=\small] {$v_k$} (ACT);
